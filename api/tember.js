...
async function createSpark(payload) {
  ...
  const row = {
    ...
    prompt_category: cleanText(payload.prompt_category).slice(0, 80),
    prompt_label: cleanText(payload.prompt_label).slice(0, 120),
    prompt_bridge: cleanText(payload.prompt_bridge).slice(0, 500),
    prompt_text: cleanText(payload.prompt_text || payload.thought).slice(0, 500),
